...
model{
  for(i in Nstart:Nend){
    ...
    X[3,i] <- PAR[i]    ## not included in interactions
    X[4,i] <- Sshall_ant[i]
    X[5,i] <- Sdeep_ant[i]
    ...
    for(j in 1:Nlag){ # covariates weeks, months into the past
      ...
      PARtemp[i,j] <- wP[j]*P_temp[i,j]
      P_temp[i,j] <- mean(PAR[(Yday[i]-C1[j]):(Yday[i]-C2[j])])
    ...
    # need to fix indices and names
    # key:
    # 1 VPD # 2 Tair # 3 PAR # 4 Sshall # 5 Sdeep
    # X1a = cbind(X1[,1]^2, X1[,2]^2)
    # X2 = X1[,1]*X1[,2], X1[,1]*X1[,4], X1[,1]*X1[,5], X1[,2]*X1[,4], X1[,2]*X1[,5], X1[,4]*X1[,5]
    dYdVPD[i] <- beta1[1] + 2*beta1a[1]*VPDant[i] + beta2[1]*TAant[i] + beta2[2]*Sshall_ant[i] + beta2[3]*Sdeep_ant[i]
    dYdT[i]   <- beta1[2] + 2*beta1a[2]*TAant[i] + beta2[1]*VPDant[i] + beta2[4]*Sshall_ant[i] + beta2[5]*Sdeep_ant[i]
    dYdP[i]   <- beta1[3]
    dYdSs[i]  <- beta1[4] + beta2[2]*VPDant[i] + beta2[4]*TAant[i] + beta2[6]*Sdeep_ant[i]
    dYdSd[i]  <- beta1[5] + beta2[3]*VPDant[i] + beta2[5]*TAant[i] + beta2[6]*Sshall_ant[i]
