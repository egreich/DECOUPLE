...
run_modSAM <- function(dataIN, varname, sitename, newinits = F, overwrite = F, lowdev = F){
  ...
  # Define filenames
  modelname <- "./models/modelSAM.R"
  initfilename <- paste("./output_modelSAM/inits/", sitename, "/inits_", varname,"_hour",".RData", sep = "")
  jm_codafilename <- paste("./output_modelSAM/coda/", sitename, "/jm_coda_", varname,".RData", sep = "")
  mcmcfoldername <- paste("./output_modelSAM/convergence/", sitename, "/", varname, sep = "")
  mcmcfilename <- paste("MCMC_", varname, sep = "")
  dffilename <- paste("./output_modelSAM/df/df_", sitename, "_", varname, ".csv", sep = "")
  
  # Create necessary directories if they do not already exist
  if(!dir.exists(paste("output_modelSAM/inits", sep = ""))){ dir.create(paste("output_modelSAM/inits", sep = ""))}
  if(!dir.exists(paste("output_modelSAM/coda", sep = ""))){ dir.create(paste("output_modelSAM/coda", sep = ""))}
  if(!dir.exists(paste("output_modelSAM/convergence", sep = ""))){ dir.create(paste("output_modelSAM/convergence", sep = ""))}
  if(!dir.exists(paste("output_modelSAM/inits/", sitename, sep = ""))){ dir.create(paste("output_modelSAM/inits/", sitename, sep = ""))}
  if(!dir.exists(paste("output_modelSAM/coda/", sitename, sep = ""))){ dir.create(paste("output_modelSAM/coda/", sitename, sep = ""))}
  if(!dir.exists(paste("output_modelSAM/convergence/", sitename, sep = ""))){ dir.create(paste("output_modelSAM/convergence/", sitename, sep = ""))}
  if(!dir.exists(mcmcfoldername)){ dir.create(mcmcfoldername)}
  if(!dir.exists(paste("output_modelSAM/df", sep = ""))){ dir.create(paste("output_modelSAM/df", sep = ""))}
...
if(test==T){
  ...
  df_mod <- dateconnect(dfobj = df_mod, datevect = YIN$DOY, datename = "DOY", identifier = NULL, varlist = c("VPDant", "TAant", "Sshall_ant", "Sdeep_ant", "PAR_ant"))
  df_mod <- dateconnect(dfobj = df_mod, datevect = YIN$TIMESTAMP, datename = "TIMESTAMP", identifier = NULL, varlist = c("VPDant", "TAant", "Sshall_ant", "Sdeep_ant", "PAR_ant"))
